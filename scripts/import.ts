...
parseStream(stream, { headers: false, delimiter: ';' })
  .on('data', function (data) {
    const newData = {
      ...
      membershipStarts: new Date(data[3].replace(/(\d{2})\.(\d{2})\.(\d{4})/, '$3-$2-$1')),
      membershipEnds: new Date(data[4].replace(/(\d{2})\.(\d{2})\.(\d{4})/, '$3-$2-$1')),
      accepted: true,
      access: 'Admin',
      id: id,
    }
